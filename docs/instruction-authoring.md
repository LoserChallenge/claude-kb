...
**A test on one machine failed to reproduce that claim, 2026-08-05.** Two contradicting one-line rules, one per surface, each forbidding the other's token, ten trivial questions per round. Round one went 10 for 10 to the output style's token. The tokens were then swapped between the files; the same token won again, now from CLAUDE.md. Round one had measured a preference for that word, not a precedence between surfaces. **Both surfaces can win; neither reliably dominated.** The swap is what caught it — a single-round version of this test yields a confident wrong answer, which is likely how the blog-grade claims in circulation were produced.

**Do not accept Claude's own account of the precedence either.** In that test Claude volunteered "project instructions outrank the output style" in the same turn it chose the token that happened to sit there. A model asserting a precedence rule is not evidence of one.

**Practical reading:** treat both surfaces as suggestions rather than configuration. The memory docs call CLAUDE.md "context, not enforced configuration" and note that contradicting rules may be resolved arbitrarily `[memory]`. Anything that must never vary belongs in a hook or a permission rule, where the harness enforces it rather than the model choosing. **That answer is the same whichever surface would have won**, which is why settling the question has little practical payoff — route by audience instead: an output style reaches the main conversation only, CLAUDE.md is the only one of the two a subagent receives.
...
**To settle it on your own machine:** put one line in a test directory's CLAUDE.md and a contradicting line in an output style set for that directory — two distinct tokens, each file forbidding the other's. Ask ten trivial questions, record which token appears per turn.

**`/context` will not confirm the style is active** — verified 2026-08-05 by running it. It reports memory files, skills, agents, tools and message volume, with no row for the output style; a style is folded into the system prompt total because it modifies the system prompt rather than loading as a separate file. Use it to confirm the test CLAUDE.md appears under Memory files, and treat the tokens themselves as the check on the style: if neither token ever appears, something failed to load and the run is void. Then swap the tokens between the files and repeat, which rules out any bias toward the literal strings. Set `keep-coding-instructions: true` so the only variable is the conflicting line. One turn proves nothing; this is a weighting, so it needs a count.
